...
local function checkForValidId( calling_ply, id )
	...
	elseif id:find( "%." ) then -- Assume IP and check
	 	if not ULib.isValidIP( id ) then
			ULib.tsayError( calling_ply, "IP inválido.", true )
			return false
		end
	elseif id:find( ":" ) then
	 	if not ULib.isValidSteamID( id ) then -- Assume steamid and check
			ULib.tsayError( calling_ply, "SteamID inválido.", true )
			return false
		end
	elseif not tonumber( id ) then -- Assume uniqueid and check
		ULib.tsayError( calling_ply, "ID único inválido", true )
		return false
	end
...
usermanagementhelp:help( "Ver as instruções de gerenciamento de usuários." )
...
function ulx.adduser( calling_ply, target_ply, group_name )
	...
	ulx.fancyLogAdmin( calling_ply, "#A adicionou #T ao cargo #s", target_ply, group_name )
end
...
adduser:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names_no_user, hint="cargo", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
adduser:defaultAccess( ULib.ACCESS_SUPERADMIN )
adduser:help( "Adiciona um usuário a um cargo específico." )
...
function ulx.adduserid( calling_ply, id, group_name )
	...
	if ULib.ucl.users[ id ] and ULib.ucl.users[ id ].name then
		ulx.fancyLogAdmin( calling_ply, "#A adicionou #s ao cargo #s", ULib.ucl.users[ id ].name, group_name )
	else
		ulx.fancyLogAdmin( calling_ply, "#A adicionou SteamID #s ao cargo #s", id, group_name )
	end
end
...
adduserid:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names_no_user, hint="cargo", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
adduserid:defaultAccess( ULib.ACCESS_SUPERADMIN )
adduserid:help( "Adiciona um usuário a um cargo específico pelo SteamID." )

function ulx.removeuser( calling_ply, target_ply )
	ULib.ucl.removeUser( target_ply:UniqueID() )

	ulx.fancyLogAdmin( calling_ply, "#A removeu todos os acessos e cargos de #T", target_ply )
end
...
removeuser:help( "Remove todos os acessos especiais do usuário." )
...
function ulx.removeuserid( calling_ply, id )
	...
	if not ULib.ucl.authed[ id ] and not ULib.ucl.users[ id ] then
		ULib.tsayError( calling_ply, "Nenhum jogador com ID \"" .. id .. "\" existe na lista de usuários ULib", true )
		return false
	end
	...
	if name then
		ulx.fancyLogAdmin( calling_ply, "#A removeu todos os acessos e cargos de #s", name )
	else
		ulx.fancyLogAdmin( calling_ply, "#A removeu todos os acessos e cargos de #s", id )
	end
end
local removeuserid = ulx.command( CATEGORY_NAME, "ulx removeuserid", ulx.removeuserid, nil, false, false, true )
removeuserid:addParam{ type=ULib.cmds.StringArg, hint="SteamID, IP, ou ID único" }
removeuserid:defaultAccess( ULib.ACCESS_SUPERADMIN )
removeuserid:help( "Remove todos os acessos especiais do usuário pelo ID." )
...
function ulx.userallow( calling_ply, target_ply, access_string, access_tag )
	...
	if not success then
		ULib.tsayError( calling_ply, string.format( "Usuário \"%s\" já tem acesso a \"%s\"", target_ply:Nick(), access_string ), true )
	else
		if not access_tag or access_tag == "" then
			ulx.fancyLogAdmin( calling_ply, "#A concedeu acesso a #q para #T", access_string, target_ply )
		else
			ulx.fancyLogAdmin( calling_ply, "#A concedeu acesso a #q com a tag #q to #T", access_string, access_tag, target_ply )
		end
	end
end
...
userallow:addParam{ type=ULib.cmds.StringArg, hint="comando" } -- TODO, add completes for this
userallow:addParam{ type=ULib.cmds.StringArg, hint="tag de acesso", ULib.cmds.optional }
userallow:defaultAccess( ULib.ACCESS_SUPERADMIN )
userallow:help( "Adiciona uma permissão específica a um usuário específico." )
...
function ulx.userallowid( calling_ply, id, access_string, access_tag )
	...
	if not success then
		ULib.tsayError( calling_ply, string.format( "Usuário \"%s\" já tem acesso a \"%s\"", name, access_string ), true )
	else
		if not access_tag or access_tag == "" then
			ulx.fancyLogAdmin( calling_ply, "#A concedeu acesso a #q para #s", access_string, name )
		else
			ulx.fancyLogAdmin( calling_ply, "#A concedeu acesso a #q com a tag #q para #s", access_string, access_tag, name )
		end
	end
end
local userallowid = ulx.command( CATEGORY_NAME, "ulx userallowid", ulx.userallowid, nil, false, false, true )
userallowid:addParam{ type=ULib.cmds.StringArg, hint="SteamID, IP, ID único" }
userallowid:addParam{ type=ULib.cmds.StringArg, hint="comando" } -- TODO, add completes for this
userallowid:addParam{ type=ULib.cmds.StringArg, hint="tag de acesso", ULib.cmds.optional }
userallowid:defaultAccess( ULib.ACCESS_SUPERADMIN )
userallowid:help( "Adiciona uma permissão específica a um usuário específico pelo ID." )
...
function ulx.userdeny( calling_ply, target_ply, access_string, should_use_neutral )
	...
	if should_use_neutral then
		if success then
			ulx.fancyLogAdmin( calling_ply, "#A definiu o acesso #q para #T como neutro", access_string, target_ply )
		else
...
userdeny:addParam{ type=ULib.cmds.StringArg, hint="comando" } -- TODO, add completes for this
...
userdenyid:addParam{ type=ULib.cmds.StringArg, hint="SteamID, IP, ou ID único" }
userdenyid:addParam{ type=ULib.cmds.StringArg, hint="comando" } -- TODO, add completes for this
...
addgroup:addParam{ type=ULib.cmds.StringArg, hint="cargo" }
addgroup:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names, hint="adquire de", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes, default="user", ULib.cmds.optional }
addgroup:defaultAccess( ULib.ACCESS_SUPERADMIN )
addgroup:help( "Cria um cargo com permissões adquiridas (opcional)." )

function ulx.renamegroup( calling_ply, current_group, new_group )
	if ULib.ucl.groups[ new_group ] then
		ULib.tsayError( calling_ply, "Este cargo já existe!", true )
		return
	end

	ULib.ucl.renameGroup( current_group, new_group )
	ulx.fancyLogAdmin( calling_ply, "#A renomeou o cargo #s para #s", current_group, new_group )
end
local renamegroup = ulx.command( CATEGORY_NAME, "ulx renamegroup", ulx.renamegroup, nil, false, false, true )
renamegroup:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names_no_user, hint="cargo atual", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
renamegroup:addParam{ type=ULib.cmds.StringArg, hint="novo cargo" }
renamegroup:defaultAccess( ULib.ACCESS_SUPERADMIN )
renamegroup:help( "Renomeia um cargo." )
...
setgroupcantarget:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names, hint="cargo", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
...
removegroup:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names_no_user, hint="cargo", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
...
function ulx.groupallow( calling_ply, group_name, access_string, access_tag )
	...
	if not success then
		...
		if not access_tag or access_tag == "" then
			ulx.fancyLogAdmin( calling_ply, "#A concedeu acesso a #q ao cargo #s", access_string, group_name )
		else
			ulx.fancyLogAdmin( calling_ply, "#A concedeu acesso a #q com a tag #q ao cargo #s", access_string, access_tag, group_name )
		end
	end
end
local groupallow = ulx.command( CATEGORY_NAME, "ulx groupallow", ulx.groupallow, nil, false, false, true )
groupallow:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names, hint="cargo", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
...
function ulx.groupdeny( calling_ply, group_name, access_string )
	...
	if success then
		ulx.fancyLogAdmin( calling_ply, "#A revogou acesso a #q ao cargo #s", access_string, group_name )
	else
...
groupdeny:addParam{ type=ULib.cmds.StringArg, completes=ulx.group_names, hint="cargo", error="Cargo especificado \"%s\" não existe" , ULib.cmds.restrictToCompletes }
