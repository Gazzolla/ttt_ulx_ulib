...
local function init()
	...
	if logFile:GetBool() then
		...
		ulx.logString( "Novo mapa: " .. game.GetMap() )
	end
end
...
local function playerConnect( name, address )
	...
	if logEvents:GetBool() then
		ulx.logString( string.format( "Jogador \"%s\" se conectou.", name ) )
	end
end
...
local function playerInitialSpawn( ply )
	...
	local txt = string.format( "Jogador \"%s\" entrou no servidor <%s | %s> (levou %i segundos).", ply:Nick(), ply:SteamID(), ply:IPAddress(), seconds )
	if logEvents:GetBool() then
		ulx.logString( txt )
	end
...
local function playerDisconnect( ply )
	local txt = string.format( "Jogador \"%s\" deixou o servidor <%s | %s>", ply:Nick(), ply:SteamID(), ply:IPAddress() )
	if logEvents:GetBool() then
		ulx.logString( txt )
	end
...
local function playerDeath( victim, weapon, killer )
	if logEvents:GetBool() then
		...
		if not killer:IsPlayer() then
			ulx.logString( string.format( "%s foi morto por %s", victim:Nick(), killer:GetClass() ) )
		elseif weapon == nil or not weapon:IsValid() then
			ulx.logString( string.format( "%s matou %s", killer:Nick(), victim:Nick() ) )
		elseif victim ~= killer then
			ulx.logString( string.format( "%s matou %s usando %s", killer:Nick(), victim:Nick(), weapon:GetClass() ) )
		else
			ulx.logString( string.format( "%s se suicidou!", victim:Nick() ) )
		end
	end
end
...
local function nameCheck( ply, oldnick, newnick )
	local msg = string.format( "%s<%s> mudou seu nome para %s", oldnick, ply:SteamID(), newnick )
...
local function plyColor( target_ply, showing_ply )
	...
	elseif logEchoColorPlayerAsGroup:GetBool() then
		return everyone_color --team.GetColor( target_ply:Team() )
	else
		return player_color
	end
end
...
local function makePlayerList( calling_ply, target_list, showing_ply, use_self_suffix, is_admin_part )
	...
	if #players > 1 and #target_list == #players then
		return { everyone_color, "todos os jogadores" }
	end
	if #target_list > 1 and (UlxCommandFilter == "innocent" or UlxCommandFilter == "!innocent") then
		return { everyone_color, (UlxCommandFilter == "innocent" and "todos os Inocentes" or "todos menos os Inocentes") }
	end
	if #target_list > 1 and (UlxCommandFilter == "traitor" or UlxCommandFilter == "!traitor")  then
		return { everyone_color, (UlxCommandFilter == "traitor" and "todos os Traidores" or "todos menos os Traidores") }
	end
	if #target_list > 1 and (UlxCommandFilter == "detective" or UlxCommandFilter == "!detective")  then
		return { everyone_color, (UlxCommandFilter == "detective" and "todos os Detetives" or "todos menos os Detetives") }
	end
	if #target_list > 1 and (UlxCommandFilter == "alive" or UlxCommandFilter == "!alive")  then
		return { everyone_color, (UlxCommandFilter == "alive" and "todos os vivos" or "todos menos os vivos") }
	end
	if #target_list > 1 and (UlxCommandFilter == "dead" or UlxCommandFilter == "!dead")  then
		return { everyone_color, (UlxCommandFilter == "dead" and "todos os mortos" or "todos menos os mortos") }
	end
	if #target_list > 1 and (UlxCommandFilter == "spec" or UlxCommandFilter == "!spec")  then
		return { everyone_color, (UlxCommandFilter == "spec" and "todos os Espectadores" or "todos menos os Espectadores") }
	end
	if #target_list > 1 and (UlxCommandFilter == "dm" or UlxCommandFilter == "!dm")  then
		return { everyone_color, (UlxCommandFilter == "dm" and "todos os jogadores do DM" or "todos menos os jogadores do DM") }
	elseif is_admin_part then
		local target = target_list[ 1 ] -- Only one target here
		if anonymous and target ~= showing_ply then
			return { everyone_color, "(Alguém)" }
	...
	for i=1, #target_list do
		...
		if target == showing_ply then
			if not use_self_suffix or calling_ply ~= showing_ply then
				table.insert( strs, "você" )
			else
				table.insert( strs, "você mesmo" )
			end
